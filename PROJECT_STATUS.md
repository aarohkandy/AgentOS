...
├── PROJECT_STATUS.md            # This file - complete project description
├── requirements.txt             # Python dependencies (pytest, etc.)
├── pyproject.toml               # Pytest configuration
│
├── docs/
│   └── oracle-vm-setup.md      # Step-by-step Oracle VirtualBox setup instructions
│
├── scripts/
│   ├── setup-passwordless-sudo.sh    # Script to disable sudo password prompts (for automation)
│   └── check-iso-build.sh           # Script to check ISO build progress
│
├── src/
│   └── agent/
│       ├── __init__.py
│       ├── types.py            # Core data structures (Message, ToolCall, AgentStep, AgentResult)
│       ├── llm_interface.py    # Abstract LLM backend interface + EchoBackend (for testing)
│       └── agent_core.py       # Core Agent class (minimal skeleton, single-step for now)
│
├── tests/
│   └── test_agent_core.py      # Unit tests for Agent (using EchoBackend)
│
└── iso-build/                  # Directory for building custom Debian-based ISO
    ├── config/                 # live-build configuration
    │   ├── package-lists/      # Custom packages to include
    │   ├── includes.chroot/    # Files to include in the ISO
    │   └── archives/           # APT repository configuration
    └── (build artifacts)        # .build/, cache/, etc. (generated by live-build)
```
